#!/bin/bash
# Runs the entire post-reconstruction chain on DATA4 to produce the final
# pipeline_data4_full_run output. Called as one background job so we don't
# have to chain task notifications across many short jobs.
#
# NOTE: This script assumes reconstruction (COLMAP + splatfacto + OpenMVS mesh)
# has ALREADY completed for data4, producing:
#   output/mesh_data4/mesh_data4_openmvs.ply
#   output/mesh_data4/scene_textured0.png
#   output/splat_data4_noinit_pruned.ply
#   nerfstudio/dense/splatfacto/<TIMESTAMP>/dataparser_transforms.json
#
# All downstream paths override the hardcoded v32_data3 defaults because
# none of these post-recon scripts support a --scene-name flag.
set -e
...
# Compute dataparser path at runtime from the newest splatfacto run.
...
# Output directory naming for data4.
...
echo "---"
ls -la output/pipeline_data4_full_run/mesh/
echo "---"
ls -la output/pipeline_data4_full_run/splat/
